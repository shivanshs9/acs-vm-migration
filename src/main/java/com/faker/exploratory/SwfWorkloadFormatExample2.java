 ...
 * web page and is located at the resources folder of this project. The workload
 * file has 18239 jobs that will be created as Cloudlets.
 *
 * <p>
 * Considering the large number of cloudlets that can have a workload file, that
 * can cause the simulation to consume a lot of resources at the developer
 * machine and can <b>spend a long time to finish</b>, the example allow to
 * limit the maximum number of cloudlets to be submitted to the
 * DatacenterBroker. See the
 * {@link #maximumNumberOfCloudletsToCreateFromTheWorkloadFile} attribute for
 * more details.
 * </p>
 *
 * <p>
 * The workload file is compressed in <i>gz</i> format and the <i>swf</i> file
 * inside it is just a text file that can be opened in any text editor. For more
 * information about the workload format, check
 * <a href="http://www.cs.huji.ac.il/labs/parallel/workload/swf.html">this
 * page</a>.
 * </p>
 *
 * <p>
 * Check important details at {@link SwfWorkloadFileReader} and
 * {@link TraceReaderAbstract}.
 * </p>
 *
 * @author Manoel Campos da Silva Filho
 * @since CloudSim Plus 1.0.0
 */
public class SwfWorkloadFormatExample2 {
    /**
    ...
    private static final int NUMBER_OF_VMS_PER_HOST = 3;
    ...
    public static void main(String[] args) {
        new SwfWorkloadFormatExample2();
    }

    private SwfWorkloadFormatExample2() {
        /*
        ...
        // + " in 5 seconds. Since it reads a workload file, it can take a long time to
        // finish.");
        // sleep(5);
        ...
        try {
            ...
            /*
             * Vms and cloudlets are created before the Datacenter and host because the
             * example is defining the hosts based on VM requirements and VMs are created
             * based on cloudlet requirements.
             */
    ...
    private List<Host> createHostsOfSameCapacity(long numberOfHosts, long numberOfPes) {
        ...
        for (int i = 0; i < numberOfHosts; i++) {
            ...
            Host host = new HostSimple(ram, bw, storage, peList).setRamProvisioner(new ResourceProvisionerSimple())
                    .setBwProvisioner(new ResourceProvisionerSimple()).setVmScheduler(vmScheduler);
