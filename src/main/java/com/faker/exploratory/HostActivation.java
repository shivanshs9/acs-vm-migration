...
/**
 * An example showing how to create a Datacenter with all Hosts powered off and
 * let the {@link VmAllocationPolicy} to power Hosts on and off according to
 * demand. For this simulation, the {@link VmAllocationPolicyBestFit} is used.
 * </p>
 *
 * <p>
 * The dynamic demand for Hosts is simulated by creating VMs during simulation
 * runtime. To enable this, it's required to define a
 * {@link Datacenter#setSchedulingInterval(double)} scheduling interval}. It's
 * also used a Event Listener to track the simulation clock. This way, when the
 * clock increases according to the scheduling interval, a new VM is created, up
 * to the total VMs defined in {@link #MAX_VMS}.
 * </p>
 *
 * <p>
 * It creates one Cloudlet for each VM and the Cloudlets are created with
 * different lengths. This enables VMs to become idle in different times. To
 * ensure idle VMs are destroyed as soon as they become idle (so that we can
 * quickly check that the number of VMs into a Host has reduced), a
 * {@link DatacenterBroker#setVmDestructionDelayFunction(Function)} is set.
 * Check the {@link #createBroker()} method for details.
 *
 * <p>
 * Since the total number of PEs requested by all VMs is equal to 24 and each
 * Host has 8 PEs, it's only used 3 Hosts to meet the demand.
 * </p>
 *
...
public class HostActivation {
     ...
     * Then, if the time defined in the {@link #SCHEDULING_INTERVAL} has passed, it
     * created another VM.
     *
    ...
    private void printHostsStatistics() {
        currentActiveHosts = datacenter0.getHostList().stream().filter(Host::isActive).count();

        System.out.printf("# %.2f: %d Active Host(s):\n", simulation.clock(), currentActiveHosts);
        datacenter0.getHostList().forEach(host -> System.out.printf("\tHost %3d | VMs: %4d | Active: %s \n",
    ...
    /**
     * Creates a Host and doesn't power it on (it will be powered on according to
     * demand).
     * 
    ...
    private Cloudlet createCloudlet(Vm vm) {
         ...
         * ensuring that a positive length is generated (Cloudlets with negative length
         * run indefinitely until being explicitly stopped.
         */
