...
public class Example1 {
	...
	private Datacenter createDatacenter() {
		...
		for (int i = 0; i < HOSTS; i++) {
			Host host = createHost(HOST_PES / (i % 2 + 1));
			hostList.add(host);
		}
	...
	private Host createHost(int pesCount) {
		final List<Pe> peList = new ArrayList<>(pesCount);
	...
	private List<Vm> createVms() {
		...
		for (int i = 0; i < VMS; i++) {
			...
			final Vm vm = new VmSimple(1000, VM_PES / (i % 2 + 1));
